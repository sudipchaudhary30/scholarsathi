import React from 'react';
import Sidebar from '../components/Sidebar'; 
import './Admindashboard.css'; 

const ScholarshipOffers = () => {
  return (
    <div className="dashboard-container">
      <Sidebar />

      <main className="main-content">
        <header>
          <h1>Scholarship Offers</h1>
          <p>Manage available scholarships for students.</p>
        </header>

        <section className="card">
          <h3>Scholarship List</h3>
          <table style={{ width: '100%', borderCollapse: 'collapse', marginTop: '15px' }}>
            <thead>
              <tr style={{ backgroundColor: '#3b82f6', color: 'white' }}>
                <th style={{ padding: '10px', border: '1px solid #ccc' }}>Scholarship ID</th>
                <th style={{ padding: '10px', border: '1px solid #ccc' }}>Mascot</th>
                <th style={{ padding: '10px', border: '1px solid #ccc' }}>Amount</th>
                <th style={{ padding: '10px', border: '1px solid #ccc' }}>Actions</th>
              </tr>
            </thead>
            <tbody>
              <tr style={{ backgroundColor: 'rgba(255,255,255,0.2)' }}>
                <td style={{ padding: '10px', border: '1px solid #ccc' }}>S001</td>
                <td style={{ padding: '10px', border: '1px solid #ccc' }}>FalconX</td>
                <td style={{ padding: '10px', border: '1px solid #ccc' }}>25,000</td>
                <td style={{ padding: '10px', border: '1px solid #ccc' }}>
                  {/* <button>Edit</button> <button>Remove</button> */}
                  <button className="btn btn-edit">Edit</button>
                  <button className="btn btn-delete">Remove</button>
                </td>
              </tr>
              <tr style={{ backgroundColor: 'rgba(255,255,255,0.2)' }}>
                <td style={{ padding: '10px', border: '1px solid #ccc' }}>S002</td>
                <td style={{ padding: '10px', border: '1px solid #ccc' }}>Phoenix</td>
                <td style={{ padding: '10px', border: '1px solid #ccc' }}>15,000</td>
                <td style={{ padding: '10px', border: '1px solid #ccc' }}>
                
                     <button className="btn btn-edit">Edit</button>
                  <button className="btn btn-delete">Remove</button>
                </td>
              </tr>
            </tbody>
          </table>
        </section>
      </main>
    </div>
  );
};

export default ScholarshipOffers;